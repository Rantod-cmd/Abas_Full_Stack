...
export function RevenueChartCard({ rows, metricsDaily, loading, dailyFinancialData }: RevenueChartCardProps) {
  const { t } = useLocale();
  const chartData =
    dailyFinancialData?.length
      ...
      : metricsDaily?.length && metricsDaily
        ? metricsDaily.map((day) => ({
          name: `Day ${day.day}`,
          revenue: (day.cogs || 0) + (day.opex || 0),
          expense: day.revenue, // Note: This fallback legacy logic seems swapped, keeping as is for safety unless requested, but dailyFinancialData is the priority.
          profit: day.net_profit,
        }))
        : rows.length > 0
          ? rows.map((row) => ({
                  ...
                  content={({ active, payload, label }) =>
                    active && payload?.length ? (
                      <div className="rounded-xl border border-[#e5e7ff] bg-white px-3 py-2 text-xs shadow-lg">
                        ...
                        <p className="text-[#4f46e5]">Revenue: ฿{formatNumber(Number(payload[1].value) || 0)}</p>
                        <p className="text-[#0996d3]">Expense: ฿{formatNumber(Number(payload[0].value) || 0)}</p>
                      </div>
                    ) : null
                  }
                />
